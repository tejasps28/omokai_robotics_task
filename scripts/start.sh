...
while (( $# > 0 )); do
  case "$1" in
    ...
    --vision|--perception)
      [[ "${mode}" == core ]] || {
        echo "Only one perception mode flag may be supplied." >&2
        exit 2
      }
      mode=vision
      ;;
    ...
    --print-config)
      ...
      ;;
    --help|-h)
      echo "Usage: $0 [--gui] [--moving-actor | --stationary-actor] [--red-actor] [--perception]"
      exit 0
      ;;
    *)
      echo "Usage: $0 [--gui] [--moving-actor | --stationary-actor] [--red-actor] [--perception]" >&2
      exit 2
      ;;
  esac
  shift
