...
DOCK_DEPARTURE_CLEARANCE_M = 1.0
...
class FleetNav2Adapter:
    ...
    def __init__(
        ...
        *,
        ...
        pose_lookup: Callable[[RobotId], Pose2D | None] | None = None,
    ) -> None:
        self._node = node
        ...
        self._split_departure_origins: dict[RobotId, Pose2D] = {}
        self._split_stop_requested = False
    ...
    def execute_split(
        ...
        *,
        ...
        """Advance queued robot work as reservations become safely available."""
        ...
        try:
            while not self._split_scheduler.complete and monotonic() < deadline:
                if self._root_failure is not None or self._split_stop_requested:
                    break
                ready = self._split_scheduler.ready_goals()
                self._split_scheduler.claim(ready)
                for goal in ready:
                    origin = self._departure_origin(goal)
                    try:
                        token = self._adapters[goal.robot_id].dispatch(
                            self._execution_goal(goal),
                            speed_mps,
                    ...
                    if origin is not None:
                        self._split_departure_origins[goal.robot_id] = origin
                ...
                if self._root_failure is None:
                    self._release_cleared_departures()
                if self._cancel_check is not None and self._cancel_check():
                    self._begin_operator_cancel()

            if self._pending or not self._split_scheduler.complete:
                if (
                    self._root_failure is None
                    and not self._split_stop_requested
                ):
                    self._begin_timeout()
                cancel_deadline = monotonic() + 5.0
                while self._pending and monotonic() < cancel_deadline:
    ...
    def _reset(self) -> None:
        ...
        self._split_final = {}
        self._split_departure_origins = {}
        self._split_stop_requested = False

    def _departure_origin(self, goal: RobotGoal) -> Pose2D | None:
        if '/outbound' not in goal.goal_id or self._pose_lookup is None:
            return None
        return self._pose_lookup(goal.robot_id)

    def _release_cleared_departures(self) -> None:
        if self._split_scheduler is None or self._pose_lookup is None:
            return
        for robot_id in self._split_scheduler.active_robot_ids:
            if (
                self._split_scheduler.active_reservation(robot_id)
                != 'dock_departure'
            ):
                continue
            origin = self._split_departure_origins.get(robot_id)
            current = self._pose_lookup(robot_id)
            if origin is None or current is None:
                continue
            travelled = hypot(current.x - origin.x, current.y - origin.y)
            if travelled < DOCK_DEPARTURE_CLEARANCE_M:
                continue
            self._split_scheduler.clear_active_reservation(robot_id)
            self._split_departure_origins.pop(robot_id, None)
            if hasattr(self._node, 'get_logger'):
                self._node.get_logger().info(
                    f'{robot_id.value} cleared the dock by '
                    f'{travelled:.2f} m; releasing the next queued robot'
                )
    ...
    def _begin_operator_cancel(self) -> None:
        if self._cancel_status:
            return
        if self._split_scheduler is not None:
            self._split_stop_requested = True
