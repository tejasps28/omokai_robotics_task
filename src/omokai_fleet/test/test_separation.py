...
class ObservedSeparationTest(unittest.TestCase):
    ...
    def test_violation_names_pair_distance_and_threshold(self) -> None:
        reason = emergency_separation_reason(poses(robot2_x=0.3))

        self.assertIn('robot1/robot2=0.300m', reason)
        self.assertIn('0.38 m', reason)

    def test_exact_threshold_is_safe(self) -> None:
        self.assertIsNone(
            emergency_separation_reason(poses(robot2_x=0.38))
        )
