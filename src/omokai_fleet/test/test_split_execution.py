...
class SplitExecutionPlanTest(unittest.TestCase):
    ...
    def test_departure_clearance_releases_next_goal_before_completion(self) -> None:
        batches = tuple(
            tuple(
                RobotGoal(
                    'executing_split',
        ...
        scheduler.clear_active_reservation(RobotId.ROBOT1)
        second = scheduler.ready_goals()
        scheduler.claim(second)

        self.assertEqual((RobotId.ROBOT2,), tuple(goal.robot_id for goal in second))
        self.assertEqual(
            (RobotId.ROBOT1, RobotId.ROBOT2),
            scheduler.active_robot_ids,
        )
        self.assertTrue(scheduler.failure_is_isolatable(RobotId.ROBOT1))

    def test_arrival_reservation_requires_goal_completion(self) -> None:
        batches = tuple(
            tuple(
                RobotGoal(
                    'executing_split',
                    f'executing_split/inbound{step}/{robot_id.value}/'
                    + ('dock' if robot_id is moving else 'hold'),
                    robot_id,
                    Pose2D(float(step), float(index), 0.0),
                )
                for index, robot_id in enumerate(RobotId)
        ...
        with self.assertRaisesRegex(ValueError, 'dock departures'):
            scheduler.clear_active_reservation(RobotId.ROBOT1)
